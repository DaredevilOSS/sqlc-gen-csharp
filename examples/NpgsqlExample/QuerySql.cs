...
public class QuerySql
{
    ...
    public readonly record struct GetCopyStatsRow(long Cnt, int CInt, string CVarchar, DateTime CDate, DateTime CTimestamp);
    public async Task<GetCopyStatsRow?> GetCopyStats()
    {
        using (var connection = NpgsqlDataSource.Create(ConnectionString))
        {
            using (var command = connection.CreateCommand(GetCopyStatsSql))
            {
                using (var reader = await command.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        return new GetCopyStatsRow
                        {
                            ...
                            CInt = reader.GetInt32(1),
                            CVarchar = reader.GetString(2),
                            CDate = reader.GetDateTime(3),
                            CTimestamp = reader.GetDateTime(4)
                        };
                    }
                }
            }
